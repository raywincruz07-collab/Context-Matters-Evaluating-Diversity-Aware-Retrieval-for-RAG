...
def main_sprint2_beir():
    """Sprint 2 BEIR evaluation: 4 retrievers Ã— 6 MMR conditions on BEIR HotpotQA.
    ...
    rouge_l are fixed to 0.0 and never computed â€” only recall@k, mrr,
    ...
    print("Loading Contriever for MMR diversity embedding â€¦")
    ...
    if args.with_generation:
        try:
            ...
            print(f"Loading NLI model: {NLI_MODEL} â€¦")
        ...
        for condition in conditions:
                ...
                try:
                    ...
                    if "qa_id" in existing_df.columns:
                        if args.with_generation:
                            ...
                            if skipped > 0:
                                print(f"    Note: {skipped} retrieval-only rows found without generation â€” will be regenerated.")
                        else:
                            completed_qa_ids = set(existing_df["qa_id"].tolist())
                            results_rows = existing_df.to_dict("records")
            ...
            for i, qa in enumerate(beir_qa):
                ...
                row: dict = {
                    ...
                    "prediction": "",
                    # answer is always "" for BEIR HotpotQA â€” do not compute
                ...
                try:
                    ...
                    # Generation + NLI faithfulness (answer field is empty â€” skip lexical metrics)
                    if args.with_generation and generator_obj:
                        context_docs = [doc for doc, _ in retrieved]
                        prediction = generator_obj.generate(
                            qa["question"],
                        ...
                        # exact_match / f1 / rouge_l intentionally left as 0.0 â€”
