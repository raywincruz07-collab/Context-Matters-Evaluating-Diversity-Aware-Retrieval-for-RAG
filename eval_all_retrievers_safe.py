...
def main():
    parser = argparse.ArgumentParser(description="Safely evaluate multiple retrievers on PubMedQA.")
    ...
    parser.add_argument("--results_dir", type=str, default="results/raw", help="Directory to save raw results.")
    ...
    for retriever_name in args.retrievers:
        ...
        csv_path = os.path.join(args.results_dir, f"{prefix}{retriever_name}_top{args.top_k}.csv")
        ...
        if os.path.exists(csv_path):
            try:
                ...
                if 'qa_id' in existing_df.columns:
                    ...
                    print(f"Resuming {retriever_name}: found {len(completed_qa_ids)} completed questions.")
            except Exception as e:
                print(f"Warning: could not read existing progress for {retriever_name}: {e}")
        ...
        try:
            ...
            for i, qa in enumerate(qa_pairs):
                ...
                row = {
                    ...
                    "temperature": FIXED_GENERATION_CONFIG["temperature"] if args.with_generation else "",
                    "max_tokens": FIXED_GENERATION_CONFIG["max_tokens"] if args.with_generation else "",
                    "prediction": "",
            ...
            if args.with_generation:
                summary_row["avg_em"] = valid_rows["exact_match"].mean() if not valid_rows.empty else 0
                summary_row["avg_f1"] = valid_rows["f1"].mean() if not valid_rows.empty else 0
                summary_row["avg_rouge_l"] = valid_rows["rouge_l"].mean() if not valid_rows.empty else 0
    ...
    sum_csv_path = os.path.join(args.results_dir, f"{prefix}summary_top{args.top_k}.csv")
    ...
    if valid_res:
        ...
        ranked_path = os.path.join(args.results_dir, f"{prefix}ranked_retriever_comparison_top{args.top_k}.csv")
